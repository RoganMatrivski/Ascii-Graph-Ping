...
pub async fn console_size_change_watcher(tx: tokio::sync::watch::Sender<AppConfig>) {
    ...
    let (mut local_term_w, mut local_term_h) = (
        u32::from(term_h) - crate::consts::BOTTOM_MARGIN,
        u32::from(term_w) - crate::consts::RIGHT_MARGIN,
    );
    ...
    loop {
        ...
        if curr_term_h != local_term_h || curr_term_w != local_term_w {
            tx.send(AppConfig {
                term_height: u32::from(term_h) - crate::consts::BOTTOM_MARGIN,
                term_width: u32::from(term_w) - crate::consts::RIGHT_MARGIN,
            })
